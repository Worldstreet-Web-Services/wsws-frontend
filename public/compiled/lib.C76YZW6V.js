...
// ../../../../../node_modules/.pnpm/cropperjs@1.6.2/node_modules/cropperjs/dist/cropper.js
var require_cropper = __commonJS({
  "../../../../../node_modules/.pnpm/cropperjs@1.6.2/node_modules/cropperjs/dist/cropper.js"(exports, module) {
...
// ../bits/src/bits.cropDialog.ts
var import_cropperjs = __toESM(require_cropper(), 1);

// ../bits/src/crop.ts
function wireCropDialog(opts) {
  var _a, _b, _c;
  if (!opts) {
  ...
  init.max = { ...init.max, megabytes: 6 };
  (_a = init.selectClicks) == null ? void 0 : _a.on("click", () => site.asset.loadEsm("bits.cropDialog", { init }));
  (_b = init.selectDrags) == null ? void 0 : _b.on("dragover", (e) => e.preventDefault());
  (_c = init.selectDrags) == null ? void 0 : _c.on("drop", async (e) => {
    e.preventDefault();
    init.source = void 0;
    for (const item of e.dataTransfer.items) {
      if (item.kind === "file" && mimeAccept.includes(item.type)) {
        ...
        const uri = await new Promise((res) => item.getAsString(res));
        if (supported(uri)) {
          init.source = uri;
          return site.asset.loadEsm("bits.cropDialog", { init });
        }
...
var imageTypes = ["jpeg", "jpg", "png", "webp", "gif", "tiff", "avif"].concat(
  isSafari() ? ["heic", "heif", "svg+xml"] : isFirefox() ? ["svg+xml"] : []
);
var mimeAccept = imageTypes.map((t) => `image/${t}`).join(",");
function supported(src) {
  var _a;
...
async function initModule(o) {
  ...
  if (!defined(o)) return;
  const opts = { ...o };
  const url = opts.source instanceof Blob ? URL.createObjectURL(opts.source) : typeof opts.source === "string" && supported(opts.source) ? URL.createObjectURL(opts.source = await (await fetch(opts.source)).blob()) : URL.createObjectURL(opts.source = await chooseImage());
  if (!url) {
    (_a = opts.onCropped) == null ? void 0 : _a.call(opts, false, "Cancelled");
    return;
  }
  ...
  await new Promise((resolve, reject) => {
    ...
    (_a2 = opts.onCropped) == null ? void 0 : _a2.call(opts, false, `Image load failed: ${url} ${e.toString()}`);
  });
  ...
  const cropper = new import_cropperjs.default(image, {
  ...
  const dlg = await domDialog({
    ...
    htmlText: `<h2>Crop image to desired shape</h2><div class="crop-view"></div><span class="dialog-actions"><button class="button button-empty cancel">cancel</button><button class="button submit">submit</button></span>`,
    ...
    onClose: () => {
      var _a2;
      URL.revokeObjectURL(url);
      (_a2 = opts.onCropped) == null ? void 0 : _a2.call(opts, false, "Cancelled");
    }
  });
  dlg.show();
  async function crop() {
    var _a2, _b;
    const view = dlg.view.querySelector(".crop-view");
    view.style.display = "flex";
    view.style.alignItems = "center";
    view.innerHTML = spinnerHtml;
    const canvas = cropper.getCroppedCanvas({
      imageSmoothingQuality: "high",
      maxWidth: (_a2 = opts.max) == null ? void 0 : _a2.pixels,
      maxHeight: (_b = opts.max) == null ? void 0 : _b.pixels
    });
    const tryQuality = (quality = 0.9) => {
      canvas.toBlob(
        (blob) => {
          var _a3, _b2;
          if (blob && blob.size < ((_b2 = (_a3 = opts.max) == null ? void 0 : _a3.megabytes) != null ? _b2 : 100) * 1024 * 1024) submit(blob);
  ...
  async function submit(cropped, err) {
    var _a2, _b;
    let redirect;
    if (cropped && opts.post) {
      const formData = new FormData();
      formData.append((_a2 = opts.post.field) != null ? _a2 : "picture", cropped);
    ...
    (_b = opts.onCropped) == null ? void 0 : _b.call(opts, cropped, err);
    opts.onCropped = void 0;
    dlg.close();
  ...
  function constrain(aspectRatio, bounds, byMax = false) {
    var _a2;
    ...
    const reduce = ((_a2 = opts.max) == null ? void 0 : _a2.pixels) ? Math.max(constrained.width, constrained.height) / opts.max.pixels : 1;
...
cropperjs/dist/cropper.js:
   ...
   *)
*/
//# sourceMappingURL=lib.C76YZW6V.js.map
