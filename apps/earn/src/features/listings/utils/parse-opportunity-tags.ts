 ...
 * Uses consistent sentence structure with keywords: web3, crypto, TSION, TSION Earn
 */
export function getOpportunityDescription(tags: ParsedOpportunityTags): string {
  ...
  if (hasSkill && !hasCategory && !hasRegion && !hasType) {
    return `Find top ${skillName} opportunities in web3 — explore remote TSION bounties, grants, and projects on TSION Earn`;
  }

  if (!hasSkill && !hasCategory && hasRegion && !hasType) {
    return `Explore ${regionName}'s best remote web3 bounties, projects and grants — earn crypto by working on real TSION projects through TSION Earn`;
  }

  if (!hasSkill && !hasCategory && !hasRegion && hasType) {
    return `Find top remote web3 ${typeLabel.toLowerCase()} on TSION — earn crypto for your skills through TSION Earn`;
  }

  if (!hasSkill && hasCategory && !hasRegion && !hasType) {
    return `Find top ${categoryName} opportunities in web3 — explore remote TSION bounties, grants, and projects on TSION Earn`;
  }

  // Two tag combinations
  if (hasSkill && hasRegion && !hasType && !hasCategory) {
    return `Find top remote ${skillName} opportunities in ${regionName} — explore TSION bounties, grants, and projects on TSION Earn`;
  }

  if (hasSkill && hasType && !hasRegion && !hasCategory) {
    return `Find top remote ${skillName} ${typeLabel.toLowerCase()} in web3 — get paid in crypto for your TSION skills on TSION Earn`;
  }

  if (hasSkill && hasCategory && !hasRegion && !hasType) {
    return `Find top remote ${skillName} ${categoryName?.toLowerCase()} opportunities in web3 — explore TSION bounties, grants, and projects on TSION Earn`;
  }

  if (hasRegion && hasType && !hasSkill && !hasCategory) {
    return `Find top remote web3 ${typeLabel.toLowerCase()} in ${regionName} — earn crypto by working on real TSION projects through TSION Earn`;
  }

  if (hasRegion && hasCategory && !hasSkill && !hasType) {
    return `Explore ${regionName}'s best remote web3 ${categoryName?.toLowerCase()} bounties, projects and grants — earn crypto by working on real TSION projects through TSION Earn`;
  }

  if (hasType && hasCategory && !hasSkill && !hasRegion) {
    return `Find top remote web3 ${categoryName?.toLowerCase()} ${typeLabel.toLowerCase()} on TSION — earn crypto for your skills through TSION Earn`;
  }

  // Three tag combinations
  if (hasSkill && hasRegion && hasType && !hasCategory) {
    return `Find top remote ${skillName} crypto ${typeLabel.toLowerCase()} in ${regionName} — work remotely on real TSION projects through TSION Earn`;
  }

  if (hasSkill && hasRegion && hasCategory && !hasType) {
    return `Find top remote ${skillName} ${categoryName?.toLowerCase()} opportunities in ${regionName} — explore TSION bounties, grants, and projects on TSION Earn`;
  }

  if (hasSkill && hasType && hasCategory && !hasRegion) {
    return `Find top remote ${skillName} ${categoryName?.toLowerCase()} ${typeLabel.toLowerCase()} in web3 — get paid in crypto for your TSION skills on TSION Earn`;
  }

  if (hasRegion && hasType && hasCategory && !hasSkill) {
    return `Find top remote web3 ${categoryName?.toLowerCase()} ${typeLabel.toLowerCase()} in ${regionName} — earn crypto by working on real TSION projects through TSION Earn`;
  }

  // Four tag combination (all tags)
  if (hasSkill && hasRegion && hasType && hasCategory) {
    return `Find top remote ${skillName} ${categoryName?.toLowerCase()} crypto ${typeLabel.toLowerCase()} in ${regionName} — work remotely on real TSION projects through TSION Earn`;
  }

  // Fallback (no tags or just global)
  return `Find top remote web3 opportunities — explore TSION bounties, grants, and projects to earn crypto through TSION Earn`;
}
