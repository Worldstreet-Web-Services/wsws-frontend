...
export function pnlCardModel(position: HlClosedPositionView): PnlCardModel {
  ...
  return {
    ...
    // Sign goes BEFORE the currency, so a loss reads "-$2.50", not "$-2.50" —
    // formatUsd prepends the "$" and would otherwise trap the minus after it.
    pnlLabel: `${isWin ? "+" : "-"}${formatUsd(Math.abs(pnl))}`,
