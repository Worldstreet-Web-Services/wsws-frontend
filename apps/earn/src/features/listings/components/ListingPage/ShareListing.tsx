...
import {
  ...
  DrawerHeader,
  DrawerTitle,
} from '@earn/components/ui/drawer';
import { useClipboard } from '@earn/hooks/use-clipboard';
import { useMediaQuery } from '@earn/hooks/use-media-query';
import { Facebook } from '@earn/svg/socials/facebook';
import { Linkedin } from '@earn/svg/socials/linkedin';
import { Telegram } from '@earn/svg/socials/telegram';
import { Whatsapp } from '@earn/svg/socials/whatsapp';
import { X } from '@earn/svg/socials/x';
...
function MainContent({ listing, grant, source }: SourceType) {
  ...
  const shareMessage = (
    ...
    let copy = '';
    if (source === 'grant') {
      copy = `Just came across this massive grant opportunity by ${grant?.sponsor?.name} on TSION Earn`;
    } else {
      if (listing?.type !== 'project') {
        copy = `Just came across this banger${!!listing?.usdValue ? ` $${listing?.usdValue}` : ''} bounty by ${listing?.sponsor?.name} on TSION Earn`;
      } else {
        copy = `Just came across this gig by ${listing?.sponsor?.name} on TSION Earn`;
      }
    }
    if (
