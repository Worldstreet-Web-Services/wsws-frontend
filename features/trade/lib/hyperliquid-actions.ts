...
import { CCTP_DOMAIN, CCTP_V2, CHAIN_ID, USDC } from "@/lib/cctp/config";
import { encodeApprove, encodeArbitrumToBaseBurn, encodeReceiveMessage } from "@/lib/cctp/cctp";
...
// The Arbitrum -> Base CCTP leg: waiting for Circle's attestation after the
// burn. Bounded — waitForAttestation returns the moment the attestation is
// ready and never runs past this window (fast transfers sign in seconds).
const CCTP_ATTESTATION_TIMEOUT_MS = 120_000;
const CCTP_ATTESTATION_INTERVAL_MS = 4_000;
// Rotated while waiting on Hyperliquid's withdrawal finalization (the long,
// couple-of-minute leg) so the UI reads as progress rather than a frozen line.
const WITHDRAWAL_WAIT_MESSAGES = [
  "Confirming your withdrawal…",
  "Your funds are on the move…",
  "Bringing it back toward your wallet…",
...
export function useHyperliquidActions(walletId: string | undefined, address: string | undefined) {
  ...
  // Finishes a withdrawal's last hop over CCTP: Hyperliquid's withdraw3 only
  // ever settles to the user's own Arbitrum wallet, so this burns that Arbitrum
  // USDC home to Base and mints it there. Both legs are gasless (Arbitrum and
  // Base are both sponsored). Bounded by design — the attestation wait returns
  // the instant Circle signs it, and the Base mint's own receipt is the
  // completion, so nothing keeps polling once the funds land.
  const routeArbitrumToBaseViaCctp = useCallback(
    async (amountRaw: bigint, onStatus?: (status: string) => void): Promise<void> => {
      if (!address || amountRaw <= 0n) return;
      // The fast-transfer fee is a few basis points; ~0.2% (min a small floor)
      // covers it. We do the Base mint ourselves, so there's no forwarding fee.
      const maxFee = amountRaw / 500n > 5_000n ? amountRaw / 500n : 5_000n;
      onStatus?.("Moving funds to your main wallet…");
      const burnTx = await sendEvmBatch(
        [
          { to: USDC.arbitrum, data: encodeApprove(amountRaw) },
          {
            to: CCTP_V2.tokenMessenger,
            data: encodeArbitrumToBaseBurn(amountRaw, address as Address, maxFee),
          },
        ],
        CHAIN_ID.arbitrum
      );
      ...
      const { message, attestation } = await waitForAttestation(
        ...
        burnTx as Hex,
        { timeoutMs: CCTP_ATTESTATION_TIMEOUT_MS, intervalMs: CCTP_ATTESTATION_INTERVAL_MS }
      );
      onStatus?.("Finishing on Base…");
      // The mint tx confirming IS the completion — no post-mint polling.
      await sendEvmBatch(
        [{ to: CCTP_V2.messageTransmitter, data: encodeReceiveMessage(message, attestation) }],
        CHAIN_ID.base
      );
    },
    [address, sendEvmBatch]
  );
  ...
  const withdraw = useCallback(
    async (
      ...
      // The platform fee (a sendAsset to the treasury), signed alongside the
      // withdrawal when one is configured. Same signing family as a dex
      // transfer. Null when no fee applies.
      ...
      if (address && startingArbitrumBalance !== null) {
        ...
        let tick = 0;
        while (Date.now() < deadline) {
          // Hyperliquid's finalization is the long part (a couple of minutes) —
          // rotate reassuring lines so the wait reads as progress, not a freeze.
          onStatus?.(WITHDRAWAL_WAIT_MESSAGES[tick % WITHDRAWAL_WAIT_MESSAGES.length]);
          tick++;
        ...
        if (creditedRaw > 0n) {
          await routeArbitrumToBaseViaCctp(creditedRaw, onStatus).catch(() => {
            // Fail soft — see the comment above this block. The funds are on the
            // user's own Arbitrum wallet either way; resumeWithdrawal retries.
          });
        }
      }
    ...
    [walletId, address, signWithdrawal, signDexTransfer, routeArbitrumToBaseViaCctp]
  );
  ...
  const resumeWithdrawal = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      await routeArbitrumToBaseViaCctp(toBaseUnits(balance, SETTLE_CHAINS.arbitrum.decimals));
    } catch {
      // Best-effort — see the comment above.
    }
  }, [walletId, address, routeArbitrumToBaseViaCctp]);
