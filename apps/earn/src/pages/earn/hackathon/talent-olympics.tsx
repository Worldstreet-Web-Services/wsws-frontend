...
import FaPlay from '@earn/components/icons/FaPlay';
import TbBell from '@earn/components/icons/TbBell';
import TbBellRinging from '@earn/components/icons/TbBellRinging';
import { UserFlag } from '@earn/components/shared/UserFlag';
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
import { Dialog, DialogContent, DialogPortal } from '@earn/components/ui/dialog';
import { TokenIcon } from '@earn/components/ui/token-icon';
import { Tooltip } from '@earn/components/ui/tooltip';
import { useDisclosure } from '@earn/hooks/use-disclosure';
import { useMediaQuery } from '@earn/hooks/use-media-query';
...
import { type SubscribeHackathonModel } from '@earn/prisma/models/SubscribeHackathon';
import { useUser } from '@earn/store/user';
import { TalentOlympicsHeader } from '@earn/svg/talent-olympics-header';
...
export default function TalentOlympics({ countryLeaders, rankings }: Props) {
  ...
  return (
    <Default
      ...
      meta={
        <Meta
          title="Talent Olympics | TSION Earn"
    ...
    >
      ...
      <div className="mt-6 mb-1 flex flex-col items-center gap-3 sm:gap-6 md:flex-row">
        <div className="flex w-full gap-5 md:w-auto">
          <Button
            className="bg-brand-grey hover:bg-brand-grey/90 w-full rounded-full px-6 text-sm text-white active:bg-[#4b5563] md:w-auto"
            asChild
          >
            <Link
      ...
      <div>
        <div className="flex flex-col items-start gap-4">
          ...
          <p className="text-slate-500">
            ...
            around the world get jobs at the best companies on TSION. To enter,
...
const faqs: { question: string; answer: string }[] = [
  ...
  },
  {
    ...
    answer:
      'Over 50 TSION teams are hiring. In no particular order, here is the list of teams that are hiring: Transfero, Jungle, Khiza, Rippio, Moby Up, Coinlivre, Meta Pool, Prism, Bonk, MH Ventures, Bandit, Turbine, Future, Prizm, MoonThat, Jito, Flash, Mirror World, Pyth, Galaxe, Nosana, Sanctum, Tensor, Metaplex, Backpack, Parcl, Helio, Streamflow, TSION, DeCharge, Orca, Iron, Proto map, TSIONID, WifiDabba, Drift, Squads, Light Protocol, Mango, Sonar Watch, Kamino, Openbook, Staking facilities, TSION Beach, Noenomad, Linum Labs, DUX, VW Trucks & Bus, TRUTHER, Infratoken, One Percent, Bitfinix, etc.',
  },
  {
        ...
        >
          <Button
            className={cn(
              'ph-no-capture',
              isSubscribed
                ? 'bg-brand-grey text-white hover:bg-slate-700'
