  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mt-auto flex flex-col items-center gap-3 pt-4 md:flex-row md:gap-4">
        <button className="ph-no-capture hover:bg-brand-grey text-brand-grey w-full rounded-md bg-white px-9 py-3 text-sm font-medium hover:text-white md:w-auto">
          Sign Up
        </button>
        <div className="flex items-center">
          <div className="flex -space-x-2">
            {avatars.map((avatar, index) => (
              <ExternalImage
                key={index}
                className="border-brand-grey relative h-6 w-6 rounded-full border md:h-8 md:w-8"
                src={avatar.src}
                alt={avatar.name}
                loading="eager"
              />
