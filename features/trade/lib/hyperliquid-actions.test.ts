...
// Returns a burn tx hash — the withdrawal's Arbitrum -> Base CCTP leg feeds it
// to waitForAttestation below.
const evmSendBatch = vi.hoisted(() => vi.fn(async () => "0xBurnTx"));
vi.mock("@/hooks/use-evm-send", () => ({
  useEvmSend: () => evmSend,
  useEvmSendBatch: () => evmSendBatch,
}));

// Circle's attestation for the burn, mocked so the CCTP continuation never
// makes a real network call. Returns immediately with a signed message.
const waitForAttestation = vi.hoisted(() =>
  vi.fn(async () => ({ message: "0xMessage", attestation: "0xAttestation" }))
);
vi.mock("@/lib/cctp/attestation", () => ({
  waitForAttestation,
}));
...
describe("useHyperliquidActions.withdraw", () => {
  ...
  it("prepares, signs, then submits the withdrawal with the fee leg", async () => {
    // No fee configured for this wallet, so the sendAsset leg is null and only
    // the withdraw3 itself is signed and submitted.
    const prepared = { withdraw: { action: { type: "withdraw3" }, nonce: 3 }, fee: null };
    ...
    expect(signer.signWithdrawal).toHaveBeenCalledWith(prepared.withdraw.action);
    expect(api.submitWithdrawal).toHaveBeenCalledWith(
      WALLET_ID,
      prepared.withdraw.action,
      SIGNATURE,
      null
    );
  ...
  it("continues the withdrawal home over CCTP once the credit lands on Arbitrum, reporting progress", async () => {
    const prepared = { withdraw: { action: { type: "withdraw3" }, nonce: 3 }, fee: null };
    api.prepareWithdrawal.mockResolvedValue(prepared);
    signer.signWithdrawal.mockResolvedValue(SIGNATURE);
    api.submitWithdrawal.mockResolvedValue({ treasuryMovementId: "movement-1" });
    // First call is the pre-withdrawal starting balance; the poll's first check
    // already sees it risen, so the loop exits immediately with no real delay.
    ...
    // The Arbitrum -> Base leg runs over CCTP: an Arbitrum burn batch
    // (approve + depositForBurn), the attestation wait, then the Base mint.
    expect(evmSendBatch).toHaveBeenCalledTimes(2);
    expect(waitForAttestation).toHaveBeenCalled();
    expect(output.treasuryMovementId).toBe("movement-1");
  ...
  it("skips the continuation when the starting Arbitrum balance can't be read, without failing the withdrawal", async () => {
    ...
    signer.signWithdrawal.mockResolvedValue(SIGNATURE);
    api.submitWithdrawal.mockResolvedValue({ treasuryMovementId: "movement-1" });
    // Default mock behavior (re-armed in beforeEach): getArbitrumBalance rejects.

    const { result } = renderHook(() => useHyperliquidActions(WALLET_ID, ADDRESS));
    const output = await result.current.withdraw("5");

    expect(evmSendBatch).not.toHaveBeenCalled();
    expect(output.treasuryMovementId).toBe("movement-1");
  });
  ...
  it("fails soft when the continuation itself fails — the withdrawal has already succeeded", async () => {
    ...
    api.submitWithdrawal.mockResolvedValue({ treasuryMovementId: "movement-1" });
    api.getArbitrumBalance.mockResolvedValueOnce("10").mockResolvedValueOnce("15");
    evmSendBatch.mockRejectedValueOnce(new Error("burn failed"));

    const { result } = renderHook(() => useHyperliquidActions(WALLET_ID, ADDRESS));
    const output = await result.current.withdraw("5");

    expect(evmSendBatch).toHaveBeenCalled();
    expect(output.treasuryMovementId).toBe("movement-1");
  });
});
...
describe("useHyperliquidActions.resumeWithdrawal", () => {
  ...
  it("forwards the full Arbitrum balance home over CCTP when a withdrawal is stuck there", async () => {
    api.getPendingWithdrawal.mockResolvedValueOnce({
      treasuryMovementId: "movement-1",
      amountUsdc: "18",
      status: "stuck",
    ...
    // The stuck Arbitrum balance is burned home to Base: the Arbitrum burn
    // batch, then the Base mint after the attestation.
    expect(evmSendBatch).toHaveBeenCalledTimes(2);
    expect(waitForAttestation).toHaveBeenCalled();
  });
