...
import { playCountdownBeep } from "@/lib/casino/chess/sound";
...
export function FinalCountdown({
  ...
  useEffect(() => {
    if (secondsLeft > COUNTDOWN_FROM) setDismissed(false);
  }, [secondsLeft]);
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-30 flex items-center justify-center">
      <div
        key={n}
        className="ws-final-count text-[7rem] font-black leading-none text-[#FF5A5A] drop-shadow-[0_2px_24px_rgba(229,72,77,0.65)]"
      >
        {n}
      </div>
      <button
        ...
        className="pointer-events-auto absolute right-3 top-3 rounded-full bg-black/50 px-2.5 py-1 text-[13px] font-semibold text-white/70 backdrop-blur-sm transition-colors hover:text-white"
      >
        ✕
      </button>
      <style>{`
