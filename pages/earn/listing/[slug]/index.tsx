...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  let listingData;
  try {
    listingData = await fetchEarnServerJson<Listing | null>(
  ...
  if (listingData?.id) {
    const prefetchPromises: Promise<void>[] = [
      queryClient.prefetchQuery({
        ...
        queryFn: async () =>
          fetchEarnServerJson<number>(
            `/api/listings/${listingData.id}/submission-count`,
          ),
      }),
    ];
    ...
    if (listingData.isWinnersAnnounced) {
      prefetchPromises.push(
        queryClient.prefetchQuery({
          ...
          queryFn: async () =>
            fetchEarnServerJson<SubmissionWithUser[]>(
              `/api/listings/${listingData.id}/winners`,
            ),
        }),
      );
    }
