import crypto from 'crypto';
import type { NextApiResponse } from 'next';

import logger from '@earn/lib/logger';
import { prisma } from '@earn/prisma';
import { safeStringify } from '@earn/utils/safeStringify';
import { getURL } from '@earn/utils/validUrl';

import { type NextApiRequestWithSponsor } from '@earn/features/auth/types';
import { withSponsorAuth } from '@earn/features/auth/utils/withSponsorAuth';
import { InviteMemberTemplate } from '@earn/features/emails/components/inviteMemberTemplate';
import { senderEmail, replyToEmail } from '@earn/features/emails/utils/fromEmails';
import { resend } from '@earn/features/emails/utils/resend';

async function sendInvites(
  req: NextApiRequestWithSponsor,
  res: NextApiResponse,
) {
  const userId = req.userId;

  logger.debug(`Request body: ${safeStringify(req.body)}`);
  logger.debug(`User ID: ${userId}`);

  const { email, memberType } = req.body;

  if (!email || !memberType) {
    logger.warn('Email and member type are required');
    return res
      .status(400)
      .json({ error: 'Email and member type are required.' });
  }

  // Convert email to lowercase to ensure consistency
  const normalizedEmail = email.toLowerCase();

  try {
    logger.debug(`Fetching user details for user ID: ${userId}`);
    const user = await prisma.user.findUnique({
      where: {
        id: userId as string,
      },
      select: {
        id: true,
        email: true,
        firstName: true,
        lastName: true,
        currentSponsor: {
          select: {
            name: true,
            id: true,
          },
        },
        role: true,
        UserSponsors: true,
      },
    });

    if (!user || !user.currentSponsor) {
      logger.warn(`Unauthorized access attempt by user ID: ${userId}`);
      return res.status(403).json({ error: 'Unauthorized' });
    }

    const currentSponsorId = user.currentSponsor.id;
    const currentSponsorMembership = user.UserSponsors.find(
      (membership) => membership.sponsorId === currentSponsorId,
    );

    if (user.role !== 'GOD' && currentSponsorMembership?.role !== 'ADMIN') {
      return res.status(403).json({ error: 'Unauthorized' });
    }

    const isBlocked = await prisma.blockedEmail.findUnique({
      where: { email: normalizedEmail },
    });

    if (isBlocked) {
      return res.status(400).json({ error: 'Blocked Email' });
    }

    const expires = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
    const token = crypto.randomBytes(32).toString('hex');

    logger.debug(`Creating user invite for email: ${normalizedEmail}`);
    await prisma.userInvites.create({
      data: {
        email: normalizedEmail,
        senderId: userId as string,
        sponsorId: user.currentSponsor.id,
        memberType,
        token,
        expires,
      },
    });

    logger.debug(`Sending invite email to: ${normalizedEmail}`);
    await resend.emails.send({
      from: senderEmail,
      to: [normalizedEmail],
      subject: `${user.firstName} has invited you to join ${user.currentSponsor.name}'s profile on TSION Earn`,
      react: InviteMemberTemplate({
        sponsorName: user.currentSponsor.name,
        senderName: `${user.firstName} ${user.lastName}`,
        link: `${getURL()}earn/signup?invite=${token}`,
      }),
      replyTo: replyToEmail,
    });

    logger.info(
      `Invite sent successfully to ${normalizedEmail} by user ${userId}`,
    );
    return res.status(200).json({ message: 'Invite sent successfully.' });
  } catch (error: any) {
    logger.error(
      `User ${userId} unable to send invite: ${safeStringify(error)}`,
    );
    return res.status(500).json({
      error: error.message,
      message: 'Error occurred while sending the invite.',
    });
  }
}

export default withSponsorAuth(sendInvites);
