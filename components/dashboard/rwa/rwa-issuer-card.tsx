  ...
  return (
    <div className="ws-card p-[18px] shadow-[inset_0_1px_0_rgba(255,255,255,0.2),0_30px_70px_-30px_rgba(0,0,0,0.85)] sm:p-[22px]">
      ...
      <div className="ws-inset mt-3.5 p-[15px]">
        {price != null ? <Line k="Price" v={formatUsd(price)} /> : null}
        {apy ? <Line k="APY" v={apy} /> : null}
        {asset.tvlUsd ? <Line k="TVL" v={formatCompactUsd(asset.tvlUsd)} /> : null}
        <Line k="KYC required" v={asset.kycRequired ? "Yes" : "No"} />
        {minInvest ? <Line k="Minimum investment" v={minInvest} /> : null}
        {asset.redemption ? <Line k="Redemption" v={asset.redemption} /> : null}
        {asset.meta?.note ? <Line k="Notes" v={asset.meta.note} /> : null}
      </div>
      ...
      {asset.issuerUrl ? (
        ...
        <p className="mt-4 text-center text-[12.5px] font-normal text-white/45">
          Contact the issuer to access this asset.
        </p>
      )}
    </div>
  );
