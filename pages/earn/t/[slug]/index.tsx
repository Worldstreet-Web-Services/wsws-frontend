...
import { ShareIcon } from '@earn/components/shared/shareIcon';
...
const FeedLoop = dynamic(
  () =>
    import('@earn/features/feed/components/FeedLoop').then((mod) => mod.FeedLoop),
  {
    ssr: false,
  },
);
import { useGetFeed } from '@earn/features/feed/queries/useGetFeed';
import { type FeedDataProps } from '@earn/features/feed/types';
import {
  GitHub,
  Linkedin,
  Twitter,
  Website,
} from '@earn/features/social/components/SocialIcons';
const AddProject = dynamic(
  () =>
    import('@earn/features/talent/components/AddProject').then(
      (mod) => mod.AddProject,
    ),
  { ssr: false },
);
import { AgentBadge } from '@earn/features/agents/components/AgentBadge';
import { ProBadge } from '@earn/features/pro/components/ProBadge';
import { ProBG } from '@earn/features/pro/components/ProBg';
import { EarnAvatar } from '@earn/features/talent/components/EarnAvatar';
import { TalentProfileBackground } from '@earn/features/talent/components/TalentProfileBackground';
const ShareProfile = dynamic(
  () =>
    import('@earn/features/talent/components/shareProfile').then(
          ...
          className={cn(
            ...
            outline
              ...
              : isPro
                ...
                : 'border-brand-grey/5 bg-brand-grey/10 text-brand-grey hover:bg-brand-grey/20',
          )}
          onClick={onClick}
...
function TalentProfile({ talent, stats, bgIndex, shouldNoIndex }: TalentProps) {
  ...
  const description = useMemo(() => {
    ...
    ).toLocaleString()} earned, ${stats?.wins || 0} wins on TSION Earn.`;

    return parts.length > 0
      ? `${parts.join(' | ')}. ${statsStr}`
      : `Talent profile on TSION Earn. ${statsStr}`;
  ...
  const title =
    talent?.firstName && talent?.lastName
      ? `${talent?.firstName} ${talent?.lastName} | TSION Earn Talent`
      : 'TSION Earn';
        ...
        <>
          <Head>
            ...
            <meta
              ...
              content={
                isPublicProfile
                  ? `${talent?.firstName} ${talent?.lastName} - Talent profile on TSION Earn`
                  : 'Talent on Superteam'
              }
            />
