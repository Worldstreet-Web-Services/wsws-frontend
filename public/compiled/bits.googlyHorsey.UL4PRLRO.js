import {
  parseSquare,
  squareFile,
  squareRank
} from "./lib.PM233RJM.js";
import "./lib.KO2KTNGK.js";

// ../bits/src/bits.googlyHorsey.ts
var DEFAULT_LAYOUT = { eyeX: -14, eyeY: -15, eyeSpacing: 15 };
var GOOGLY_EYE_LAYOUTS = {
  default: DEFAULT_LAYOUT,
  cburnett: DEFAULT_LAYOUT,
  merida: { eyeX: -14, eyeY: -15, eyeSpacing: 15 },
  alpha: { eyeX: -4, eyeY: -18, eyeSpacing: 15 },
  pirouetti: { eyeX: -4, eyeY: -18, eyeSpacing: 15 },
  chessnut: { eyeX: -10, eyeY: -8, eyeSpacing: 15 },
  chess7: { eyeX: -10, eyeY: -15, eyeSpacing: 20 },
  reillycraig: { eyeX: -2, eyeY: -30, eyeSpacing: 15 },
  companion: { eyeX: -10, eyeY: -10, eyeSpacing: 15 },
  riohacha: { eyeX: -2, eyeY: -25, eyeSpacing: 15 },
  kosal: DEFAULT_LAYOUT,
  leipzig: DEFAULT_LAYOUT,
  fantasy: DEFAULT_LAYOUT,
  spatial: DEFAULT_LAYOUT,
  celtic: DEFAULT_LAYOUT,
  california: { eyeX: -2, eyeY: -15, eyeSpacing: 15 },
  caliente: { eyeX: -2, eyeY: -15, eyeSpacing: 15 },
  pixel: DEFAULT_LAYOUT,
  firi: { eyeX: -10, eyeY: -8, eyeSpacing: 15 },
  rhosgfx: DEFAULT_LAYOUT,
  maestro: { eyeX: -20, eyeY: -15, eyeSpacing: 15 },
  fresca: { eyeX: -14, eyeY: -12, eyeSpacing: 15 },
  cardinal: DEFAULT_LAYOUT,
  gioco: { eyeX: -3, eyeY: -15, eyeSpacing: 15 },
  tatiana: { eyeX: -3, eyeY: -15, eyeSpacing: 15 },
  staunty: { eyeX: -10, eyeY: -19, eyeSpacing: 15 },
  cooke: { eyeX: -2, eyeY: -25, eyeSpacing: 15 },
  monarchy: { eyeX: -10, eyeY: -20, eyeSpacing: 15 },
  governor: { eyeX: -5, eyeY: -18, eyeSpacing: 15 },
  dubrovny: DEFAULT_LAYOUT,
  "shahi-ivory-brown": { eyeX: -6, eyeY: -18, eyeSpacing: 15 },
  icpieces: { eyeX: -10, eyeY: -8, eyeSpacing: 15 },
  mpchess: { eyeX: -10, eyeY: -20, eyeSpacing: 15 },
  "kiwen-suwi": { eyeX: 3, eyeY: -15, eyeSpacing: 16 },
  horsey: { eyeX: 4, eyeY: -15, eyeSpacing: 15 },
  anarcandy: { eyeX: -22, eyeY: -11, eyeSpacing: 18 },
  xkcd: DEFAULT_LAYOUT,
  shapes: { eyeX: -10, eyeY: -15, eyeSpacing: 20 },
  letter: { eyeX: 0, eyeY: -8, eyeSpacing: 15 },
  disguised: { eyeX: 0, eyeY: -8, eyeSpacing: 15 }
};
var mousePos = { x: 0.5, y: 0.5 };
var rafId;
var chessground;
var requestRedraw;
function onMouseMove(e) {
  if (!chessground || !requestRedraw) return;
  const rect = chessground.state.dom.bounds();
  if (!rect) return;
  mousePos = {
    x: (e.clientX - rect.left) / rect.width,
    y: (e.clientY - rect.top) / rect.height
  };
  if (rafId === void 0) {
    rafId = requestAnimationFrame(() => {
      rafId = void 0;
      requestRedraw == null ? void 0 : requestRedraw();
    });
  }
}
async function initModule({ cg, redraw }) {
  enableGooglyEyesTracking(cg, redraw);
  return {
    makeGooglyShapes
  };
}
function enableGooglyEyesTracking(cg, redraw) {
  disableGooglyEyesTracking();
  chessground = cg;
  requestRedraw = redraw;
  document.addEventListener("mousemove", onMouseMove);
}
function disableGooglyEyesTracking() {
  if (rafId !== void 0) cancelAnimationFrame(rafId);
  rafId = void 0;
  chessground = void 0;
  requestRedraw = void 0;
  document.removeEventListener("mousemove", onMouseMove);
}
function makeGooglyShapes() {
  var _a, _b;
  if (!chessground) return [];
  const orientation = chessground.state.orientation;
  const pieceSet = (_a = document.body.dataset["pieceSet"]) != null ? _a : "default";
  const layout = (_b = GOOGLY_EYE_LAYOUTS[pieceSet]) != null ? _b : DEFAULT_LAYOUT;
  const knightKeys = [...chessground.state.pieces.entries()].filter(([_, piece]) => piece.role === "knight").map(([key, _]) => key);
  return knightKeys.map((key) => ({
    orig: key,
    customSvg: { html: renderGooglySvg(parseSquare(key), orientation, layout) }
  }));
}
function renderGooglySvg(square, bottomColor, layout) {
  const { eyeX, eyeY, eyeSpacing } = layout;
  const { x: mx, y: my } = mousePos;
  const file = squareFile(square);
  const rank = squareRank(square);
  const isBlackAtBottom = bottomColor === "black";
  const eyeCenter = isBlackAtBottom ? { x: (7.5 - file) / 8, y: (rank + 0.5) / 8 } : { x: (file + 0.5) / 8, y: (7.5 - rank) / 8 };
  let dx = mx - eyeCenter.x;
  let dy = my - eyeCenter.y;
  const len = Math.hypot(dx, dy) || 1;
  const maxOffset = 4;
  dx = dx / len * maxOffset;
  dy = dy / len * maxOffset;
  const leftEyeX = eyeX - eyeSpacing / 2;
  const rightEyeX = eyeX + eyeSpacing / 2;
  const lx = leftEyeX + dx;
  const ly = eyeY + dy;
  const rx = rightEyeX + dx;
  const ry = eyeY + dy;
  return `<g class="googly-eyes" transform="translate(50, 50)"><circle cx="${leftEyeX}" cy="${eyeY}" r="8" fill="white" stroke="#333" stroke-width="1.5"/><circle cx="${lx}" cy="${ly}" r="5" fill="#222"/><circle cx="${lx - 2}" cy="${ly - 2.5}" r="1.5" fill="white" opacity="0.9"/><circle cx="${rightEyeX}" cy="${eyeY}" r="8" fill="white" stroke="#333" stroke-width="1.5"/><circle cx="${rx}" cy="${ry}" r="5" fill="#222"/><circle cx="${rx - 2}" cy="${ry - 2.5}" r="1.5" fill="white" opacity="0.9"/></g>`;
}
export {
  disableGooglyEyesTracking,
  enableGooglyEyesTracking,
  initModule,
  makeGooglyShapes
};
//# sourceMappingURL=bits.googlyHorsey.UL4PRLRO.js.map
