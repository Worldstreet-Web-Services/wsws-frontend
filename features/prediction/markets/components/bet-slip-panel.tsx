...
export function BetSlipPanel({
  ...
  return (
    <section className="overflow-hidden rounded-[12px] border border-white/10 bg-[#111114] shadow-[0_24px_80px_rgba(0,0,0,0.48)]">
      <header className="flex min-h-14 items-center border-b border-white/8 bg-[linear-gradient(180deg,#242429_0%,#1b1b1f_100%)] px-4">
        <div className="flex items-center gap-2">
          <h2 className="text-[16px] font-extrabold text-white">Bet slip</h2>
          <span className="grid min-w-6 place-items-center rounded-full bg-white/12 px-1.5 py-0.5 text-[10px] font-black text-white/70">
            {selections.length}
          </span>
        </div>
        {activeTab === "singles" && selections.length > 0 ? (
          <button
            type="button"
            onClick={onClear}
            className="ml-auto cursor-pointer text-[10px] font-bold text-white/42 transition-colors hover:text-white"
          >
            Remove all
          </button>
        ) : null}
      ...
      <div className="grid grid-cols-2 border-b border-white/8 bg-black/20 p-1.5">
        <button
          type="button"
          onClick={() => onTabChange("singles")}
          className={`h-10 cursor-pointer rounded-[7px] text-[11px] font-extrabold transition-colors ${
            activeTab === "singles"
      ...
      {activeTab === "singles" ? <BookingCodeLookup onLoaded={onTicketLoaded} /> : null}
      ...
      {activeTab === "cashout" ? (
        ...
        <>
          <div className="max-h-[330px] [scrollbar-width:thin] overflow-y-auto">
            {selections.map((selection) => (
              <article key={selection.id} className="border-b border-white/7 px-4 py-3">
                <div className="flex items-start gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[10px] font-semibold text-white/38">
                      {selection.eventTitle}
                    </p>
                    <p className="mt-1 line-clamp-2 text-[12px] leading-4 font-bold text-white/78">
          ...
          <div className="space-y-3 p-4">
            <div>
              <div className="mb-2 flex items-center justify-between">
                <label htmlFor={stakeInputId} className="text-[10px] font-bold text-white/42">
                  Stake per selection
                </label>
                <span className="text-[9px] font-semibold text-white/28">pUSD</span>
              </div>
              <div className="flex h-11 items-center rounded-[8px] border border-white/10 bg-black/30 px-3 focus-within:border-white/28">
                ...
                <input
                  ...
                  className="min-w-0 flex-1 bg-transparent px-2 text-right text-[15px] font-extrabold text-white tabular-nums outline-none"
              ...
              <div className="mt-2 grid grid-cols-4 gap-1.5">
                {QUICK_STAKES.map((value) => (
                  <button
                    ...
                    className="h-8 cursor-pointer rounded-[6px] border border-white/8 bg-white/[0.035] text-[10px] font-bold text-white/52 hover:bg-white/8 hover:text-white"
                  >
                    ${value}
                  </button>
                ))}
            ...
            <dl className="space-y-2 border-t border-white/7 pt-3 text-[11px]">
              <div className="flex items-center justify-between">
                <dt className="text-white/40">Total stake</dt>
                <dd className="font-bold text-white/78 tabular-nums">{formatUsdE6(totalE6)}</dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-white/52">Estimated return</dt>
                <dd className="text-[14px] font-black text-white tabular-nums">
            ...
            <button
              ...
              className="h-12 w-full cursor-pointer rounded-[8px] bg-[linear-gradient(180deg,#dedee2_0%,#b4b4ba_100%)] text-[12px] font-black text-black transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-45"
            >
              {actionLabel}
            </button>
          </div>
