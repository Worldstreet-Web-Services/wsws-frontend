// What a Solana RWA purchase needs before it can run, and the bridge leg that
// supplies it. Pure: no wallet, no network. The panel renders from this and the
// hook executes it.
//
// Solana is gasless for this app: every transaction we sign there routes
// through the fee-sponsorship path, which pays the network fee and prefunds
// token-account rent. So the only thing a purchase can be short of is USDC,
// and the only leg is a LI.FI route whose transaction executes on Base, where
// the user's sponsored send pays no gas either.
...
export interface FundingPlan {
  // USDC to move Base -> Solana so the buy can pay for itself. An estimate:
  // the hook re-sizes it against the live quote before sending.
  bridgeUsdc: number;
...
export function planSolanaFunding(input: FundingInputs): FundingPlan | null {
  ...
  return {
    bridgeUsdc: sizeBridgeSend(shortfall),
...
export function planAffordable(plan: FundingPlan, baseUsdc: number): boolean {
  return baseUsdc + 1e-9 >= plan.bridgeUsdc;
}
...
export function planSignature(plan: FundingPlan): string {
  return plan.bridgeUsdc.toFixed(2);
}
