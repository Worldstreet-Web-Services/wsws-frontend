...
function headline(outcome: ChessGameOverOutcome, winner: Seat | null): string {
  if (outcome === "win") return "You Won!";
  if (outcome === "loss") return "You Lost";
  if (outcome === "draw") return "Game Drawn";
  if (outcome === "aborted") return "Game Aborted";
  return winner ? `${winner.name} won` : "Game over";
}

function reasonCopy(reason: ChessGameOverReason, winner: Seat | null, loser: Seat | null): string {
  switch (reason) {
    case "checkmate":
      return winner ? `${winner.name} won by checkmate` : "By checkmate";
    case "resignation":
      return loser ? `${loser.name} resigned` : "By resignation";
    case "timeout":
      return loser ? `${loser.name} ran out of time` : "On time";
    case "stalemate":
      return "By stalemate";
    case "agreement":
      return "By agreement";
    ...
    case "timeout_insufficient":
      return "Time expired without mating material";
    case "variant_end":
      return "By the variant rules";
    case "aborted":
      return "No result was recorded";
    case "unknown":
      return winner ? `${winner.name} won the game` : "The game is complete";
  }
}
...
function newGameLabel(match: ChessMatch): string {
  if (match.clockMode === "unlimited") return "New game";
  const first = match.timeControl.split("+")[0] ?? "";
  return /^\d+$/.test(first) ? `New ${first} min` : "New game";
}

function coachMessage(outcome: ChessGameOverOutcome, analysisReady: boolean): string {
  if (analysisReady) return "Your Game Review is ready. See where the game was decided.";
  if (outcome === "win") return "Strong finish. Review the turning points while they are fresh.";
  if (outcome === "loss") return "Every loss has a lesson. Find the move that changed the game.";
  if (outcome === "draw")
    return "A balanced fight. See where either side could have broken through.";
  if (outcome === "spectator") return "Review the critical positions and best alternatives.";
  return "This game ended before a result could be recorded.";
}
...
export function ChessGameOverModal({
  ...
  const rematch = async () => {
    ...
    try {
      ...
      setActionError(error instanceof Error ? error.message : "Unable to request a rematch.");
    }
  };
          ...
          <header className={styles.header}>
            <div className={styles.headlineRow}>
              ...
              <h2 id="ark-chess-game-over-title" className={styles.title}>
                {headline(presentation.outcome, winner)}
              </h2>
            </div>
            <p className={styles.reason}>{reasonCopy(presentation.reason, winner, loser)}</p>
          </header>
          <div className={styles.metrics}>
            ...
            <div className={styles.metric}>
              ...
              <div className={styles.metricCopy}>
                ...
                <p className={styles.metricLabel}>Final score</p>
              </div>
            </div>
          </div>
          <div className={styles.analysisStage}>
            <div className={styles.coachRow}>
              ...
              <div className={styles.coachBubble}>
                {coachMessage(presentation.outcome, analysisReady)}
              </div>
            </div>
            {presentation.outcome !== "aborted" ? (
              <div className={styles.counters}>
                <Counter tone="good" value={counters?.bestAndGood ?? null} label="Best / good" />
                <Counter
                  tone="mistake"
                  value={counters?.mistakesAndInaccuracies ?? null}
                  label="Mistakes"
                />
                <Counter tone="blunder" value={counters?.blunders ?? null} label="Blunders" />
              </div>
            ) : null}
          </div>
          <div className={styles.gameMeta}>
            {viewerSeat ? <span>You {formatClock(viewerSeat.clock)}</span> : null}
          ...
          <footer className={styles.footer}>
            ...
            <div className={styles.secondaryActions}>
              <button type="button" className={styles.secondaryAction} onClick={onNewGame}>
                <span aria-hidden="true">+</span> {newGameLabel(match)}
              </button>
              <button
                type="button"
              ...
              >
                <span aria-hidden="true">↻</span>
                {rematching ? "Requesting..." : rematchLabel}
              </button>
            </div>
          </footer>
        </motion.section>
