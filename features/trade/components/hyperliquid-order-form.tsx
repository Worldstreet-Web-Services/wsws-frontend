...
export function HyperliquidOrderForm({
  ...
  // What the trader stands to make or lose if their brackets hit — shown live
  // as they type TP/SL so the stakes are on the table before they commit. The
  // entry basis matches triggerValidationError's: a market order projects off
  // the live mark, a limit off the entered limit. Only a trigger on the
  // correct side of entry (a TP above entry for a long, an SL below) reads as
  // a gain/loss; a crossed value is left to triggerValidationError to flag,
  // not shown here as a nonsensical figure.
  const entryReference = orderKind === "limit" ? Number(limitPrice) || 0 : markPrice;
  const projectionInput = {
    side,
    entryPrice: entryReference,
    sizeBaseUnits,
    marginUsd: amountUsdcNum,
  };
  const tpProjection = showTriggers
    ? projectTriggerPnl({ ...projectionInput, triggerPrice: Number(takeProfitPrice) || 0 })
    : null;
  const slProjection = showTriggers
    ? projectTriggerPnl({ ...projectionInput, triggerPrice: Number(stopLossPrice) || 0 })
    : null;
  ...
  // Reward-to-risk as "1 : N" — the ratio traders judge a setup by. Only
  // meaningful when both legs are set and on the right side of entry.
  const rewardToRisk =
  ...
  return (
    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        {showTriggers && (takeProfitGain || stopLossRisk) ? (
          <div className="ws-inset mt-3 flex flex-col gap-2 p-3">
            {takeProfitGain ? (
              <div className="flex items-center justify-between text-[12.5px] font-normal">
                <span className="text-white/55">If take profit hits</span>
                <span className="tnum text-up font-semibold">
                  +{formatUsd(takeProfitGain.pnlUsd)}{" "}
                  <span className="text-up/70 font-normal">
                    ({formatSignedPercent(takeProfitGain.roePct)})
                  </span>
                </span>
              </div>
            ) : null}
            {stopLossRisk ? (
              <div className="flex items-center justify-between text-[12.5px] font-normal">
                <span className="text-white/55">If stop loss hits</span>
                <span className="tnum text-down font-semibold">
                  -{formatUsd(Math.abs(stopLossRisk.pnlUsd))}{" "}
                  <span className="text-down/70 font-normal">
                    ({formatSignedPercent(stopLossRisk.roePct)})
                  </span>
                </span>
              </div>
            ) : null}
            {rewardToRisk ? (
              <div className="flex items-center justify-between border-t border-white/8 pt-2 text-[11.5px] font-normal text-white/45">
                <span>Reward / risk</span>
                <span className="tnum">1 : {rewardToRisk.toFixed(2)}</span>
              </div>
            ) : null}
          </div>
        ) : null}
        ...
        <button
          ...
          className={`mt-3 w-full cursor-pointer rounded-[14px] p-3.75 text-[15px] font-semibold transition-opacity disabled:cursor-not-allowed disabled:opacity-50 ${
