...
import MdArrowForward from '@earn/components/icons/MdArrowForward';
...
import { recentEarnersQuery } from '@earn/features/listings/queries/recent-earners';
import { yourBookmarksQuery } from '@earn/features/listings/queries/your-bookmarks';
import { type Listing } from '@earn/features/listings/types';
import { isEligiblePeopleType } from '@earn/features/membership/utils/peopleEligibility';
import { ProIntro } from '@earn/features/pro/components/ProIntro';
import { useProUpgradeFlow } from '@earn/features/pro/state/pro-upgrade-flow';
...
const VibeCard = dynamic(() =>
  import('@earn/features/home/components/VibeCard').then((mod) => mod.VibeCard),
);

const LiveListings = dynamic(() =>
  import('@earn/features/home/components/LiveListings').then(
...
const SectionHeader = ({
  ...
  <div className="flex items-center justify-between">
    ...
    <Link
      href={href}
      className="text-brand-grey flex items-center text-xs font-semibold"
      onClick={onLinkClick}
    >
      View All
...
export const HomeSideBar = ({ type }: SideBarProps) => {
  ...
  const hasAuthenticatedSession = ready && authenticated;
