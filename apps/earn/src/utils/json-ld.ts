...
import { AppConfig } from '@earn/utils/AppConfig';
...
/**
 * Generate Organization schema for TSION Earn
 */
export function generateOrganizationSchema(): OrganizationSchema {
  ...
  return {
    ...
    logo: {
      '@type': 'ImageObject',
      url: `${baseUrl}assets/tsion-logo.png`,
    },
...
export function generateRegionalOrganizationSchema(region: {
  ...
  return {
    '@context': 'https://schema.org',
    '@type': 'Organization',
    name: `TSION Earn ${region.displayValue}`,
    url: `${baseUrl}earn/regions/${region.slug}/`,
    description: `TSION Earn ${region.displayValue} - Discover bounties and grants in the ${region.displayValue} crypto community`,
  };
}
  ...
  const organizations: OrganizationSchema[] = superteams
    ...
    .map((st) => {
      ...
      return {
        ...
        description: `${st.name} - TSION and Web3 talent community in ${areaServed}. Find crypto bounties, blockchain jobs, and grants.`,
        sameAs: sameAs.length > 0 ? sameAs : undefined,
      };
    });
