...
export function HyperliquidProPerps({ initialSymbol = "" }: HyperliquidProPerpsProps) {
  ...
  // A top-up or withdrawal moves the perps margin balance and the user's main
  // (spot) balance — refresh both the moment the perps side confirms, instead
  // of leaving it to catch up on its own poll. It does NOT touch positions or
  // orders, so only the clearinghouse balance is refetched, not the whole
  // desk. The perps-side settlement itself (the confirmation window, then the
  // on-chain leg back to the main wallet) is real transfer time that can't be
  // sped up; this just removes the extra, avoidable lag on top of it.
  const handleWalletChanged = () => {
    void trading.refetchClearinghouse();
    void portfolio.refetchFresh("all");
  };
  ...
  const handleEditTrigger = (
    ...
    withBusy(async () => {
      try {
        ...
        // A trigger edit only adds/replaces a resting TP/SL order — the
        // position size and margin are untouched, so just the orders refresh.
        void trading.refetchOrders();
      }
    });
  ...
  const handleCancelOrder = (order: HlOrderRow) =>
    withBusy(async () => {
      try {
        ...
        // Cancelling a resting order changes only the orders list.
        void trading.refetchOrders();
      }
    });
