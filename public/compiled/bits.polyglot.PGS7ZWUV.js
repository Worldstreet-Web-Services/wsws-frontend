...
// ../bits/src/bits.polyglot.ts
async function initModule(o) {
  if (!o) return hashBoard;
  const book = "bytes" in o ? makeBookPolyglot(o.bytes) : "pgn" in o ? await makeBookPgn(o.pgn, o.ply, o.progress) : { getMoves: o.getMoves };
  return {
    ...book,
    cover: o.cover ? await makeCover(book.getMoves, typeof o.cover === "object" ? o.cover.boardSize : 192, book.positions) : void 0
  };
}
async function makeBookPgn(blob, maxPly, progress, filter) {
  let bigMap = /* @__PURE__ */ new Map();
  for await (const game of pgnFromBlob(blob, 8 * 1024 * 1024, progress)) {
    ...
    traverseTree(pgn_exports.startingPosition(game.headers).unwrap(), game.moves, maxPly);
  }
  const posMap = /* @__PURE__ */ new Map();
  for (const [hash, map] of bigMap) {
    const moves = Array.from(map, ([uci, weight]) => ({ uci, weight }));
    const sum = moves.reduce((sum2, m) => sum2 + m.weight, 0);
  ...
  function traverseTree(chess, node, plyToGo) {
    var _a, _b;
    if (plyToGo === 0) return;
    const zobrist = hashChess(chess);
    const moves = (_a = bigMap.get(zobrist)) != null ? _a : /* @__PURE__ */ new Map();
    if (plyToGo > 1)
      for (const nextNode of node.children) {
        const move = san_exports.parseSan(chess, nextNode.data.san);
        if (!move) continue;
        const nextChess = chess.clone();
        const uci = makeUci(move);
        moves.set(uci, ((_b = moves.get(uci)) != null ? _b : 0) + 1);
        nextChess.play(move);
...
function makeBookPolyglot(bytes) {
  var _a;
  ...
  for (let i = 0; i < bytes.byteLength - 15; i += 16) {
    ...
    const moves = (_a = book.get(hash)) != null ? _a : [];
...
async function* pgnFromBlob(blob, chunkSize, progress) {
  const totalSize = blob.size;
  let offset = 0;
  let residual = "";
  while (offset < totalSize) {
    if ((progress == null ? void 0 : progress(offset, totalSize)) === false) {
      yield void 0;
      return;
    }
    ...
    const wholePgnsChunk = offset + chunk.size === totalSize || Math.max(crlfLast, lfLast) === -1 ? textChunk : textChunk.slice(0, Math.max(lfLast + 2, crlfLast + 4));
    const games = pgn_exports.parsePgn(residual + wholePgnsChunk).filter((game) => {
...
async function makeCover(polyglotBook, boardSize, numMoves) {
  var _a, _b, _c, _d;
  ...
  const composition = { boards: 1, squares: /* @__PURE__ */ new Map() };
  ...
  await traverseTree(Chess.default(), 4);
  for (const [sq, comp] of composition.squares) {
    const [x, y] = [squareFile(sq) * squareSize, (7 - squareRank(sq)) * squareSize];
    for (const color of comp.keys()) {
      for (const role of (_b = (_a = comp.get(color)) == null ? void 0 : _a.keys()) != null ? _b : []) {
        ctx.globalAlpha = 0.4 + 0.6 * ((_d = (_c = comp.get(color)) == null ? void 0 : _c.get(role)) != null ? _d : 0) / composition.boards;
  ...
  async function traverseTree(chess, plyToGo) {
    var _a2;
    if (plyToGo === 0) return;
    for (const om of await polyglotBook(chess)) {
      const { move } = (_a2 = normalMove(chess, om.uci)) != null ? _a2 : {};
      if (!move) continue;
      const nextChess = chess.clone();
      nextChess.play(move);
  ...
  function composeBoard(chess) {
    [...chess.board.occupied].forEach((sq) => {
      var _a2, _b2;
      const { color, role } = (_a2 = chess.board.get(sq)) != null ? _a2 : {};
      if (!color || !role) return;
      if (!composition.squares.has(sq)) composition.squares.set(sq, /* @__PURE__ */ new Map());
      const sqcomp = composition.squares.get(sq);
      if (!sqcomp.has(color)) sqcomp.set(color, /* @__PURE__ */ new Map());
      const colorMap = sqcomp.get(color);
      colorMap.set(role, ((_b2 = colorMap.get(role)) != null ? _b2 : 0) + 1);
    });
    composition.boards++;
  }
}
function makeImage(svg) {
  const src = "data:image/svg+xml;base64," + btoa(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 45 45">${svg}</svg>`);
  const img = new Image();
  img.src = src;
  return img;
}
function getMoves(book) {
  return (pos) => {
    var _a;
    const moves = (_a = book.get(hashChess(pos))) != null ? _a : [];
    const sum = moves.reduce((sum2, m) => sum2 + m.weight, 0);
    return Promise.resolve(moves.map((m) => ({ uci: m.uci, weight: m.weight / sum })));
  };
}
function shortToUci(move) {
  return FILE_NAMES[move >>> 6 & 7] + RANK_NAMES[move >>> 9 & 7] + FILE_NAMES[move & 7] + RANK_NAMES[move >>> 3 & 7] + promotes[move >>> 12 & 7];
}
function charDiff(a, b) {
  return a.charCodeAt(0) - b.charCodeAt(0);
}
function uciToShort(uci) {
...
function normalMove(chess, unsafeUci) {
  const unsafe = parseUci(unsafeUci);
  const move = unsafe && "from" in unsafe ? { ...unsafe, ...normalizeMove(chess, unsafe) } : void 0;
  return move && chess.isLegal(move) ? { uci: makeUci(move), move } : void 0;
}
var promotes = ["", "n", "b", "r", "q", "?", "?", "?"];
var pieces = {
  black: {
    bishop: makeImage(
...
//# sourceMappingURL=bits.polyglot.PGS7ZWUV.js.map
