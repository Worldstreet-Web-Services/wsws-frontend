...
const MODE_LABEL: Record<HlAbstractionMode, string> = {
  disabled: "Manual",
  unifiedAccount: "Unified",
  portfolioMargin: "Portfolio",
};
const MODE_CYCLE: HlAbstractionMode[] = ["disabled", "unifiedAccount", "portfolioMargin"];

// The full order ticket: settings pills (margin mode, leverage, HyperCore
// account-abstraction mode), entry (market or limit) with optional TP/SL,
// and the wallet's deposit/withdraw actions — one card, matching
// Hyperliquid's own pro layout. Wallet balance and account-mode status live
// here rather than in separate cards because that's how the reference
// layout treats them: settings and money, one panel.
export function HyperliquidOrderForm({
  ...
  const [modeStatus, setModeStatus] = useState<HlAbstractionModeStatus | null>(null);
  ...
  useEffect(() => {
    ...
    onGetMode()
      .then((result) => {
        if (!cancelled) setModeStatus(result);
      })
      .catch(() => {
        // Read-only status check — a failure here just leaves the pill
        // showing no fetched value rather than surfacing an error for
  ...
  const cycleAccountMode = async () => {
    if (!walletReady || modeSwitching || busy) return;
    const current = modeStatus?.mode ?? "disabled";
    const next = MODE_CYCLE[(MODE_CYCLE.indexOf(current) + 1) % MODE_CYCLE.length];
    setModeSwitching(true);
    try {
      await onSetMode(next);
      setModeStatus(await onGetMode());
    } catch (error) {
      setStatus({ text: friendlyError(error, "Could not switch account mode."), kind: "error" });
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isInsufficientMarginDetails(details)) {
        setStatus({
          text: `Still short on balance after automatically topping up (have $${details.withdrawableUsdc}, need $${details.requiredUsdc}) — deposit more, then retry.`,
          kind: "error",
        });
        return;
      }
      if (isBridgeMinimumDetails(details)) {
        setStatus({
          text: `Insufficient balance — minimum is $${details.minDepositUsdc} (you have $${details.arbitrumBalanceUsdc}). Deposit more, then retry.`,
          kind: "error",
        });
        return;
      }
  ...
  return (
    <div className="ws-card p-4 sm:p-5">
      {/* Settings pills: margin mode + leverage open the same compact panel;
          account mode cycles and applies immediately. */}
      <div className="grid grid-cols-3 gap-2">
        ...
        <button
          onClick={() => void cycleAccountMode()}
          disabled={!walletReady || modeSwitching || busy}
          className="cursor-pointer rounded-xl border border-white/10 bg-white/4 py-2.5 text-[13px] font-semibold text-white transition-colors hover:border-white/25 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {MODE_LABEL[modeStatus?.mode ?? "disabled"]}
        </button>
      </div>
      {modeStatus && !modeStatus.eligibleForBuilderFees ? (
        <p className="mt-1.5 text-[11.5px] font-normal text-white/45">
          Manual mode keeps your trades supporting builder-fee revenue.
        </p>
      ) : null}
            ...
            {(["cross", "isolated"] as HlMarginMode[]).map((mode) => (
              <button
                ...
                className={`cursor-pointer rounded-md px-2.5 py-1 text-[11.5px] font-semibold capitalize transition-colors ${
                  marginMode === mode ? "bg-white/10 text-white" : "text-white/45 hover:text-white/70"
                }`}
              >
                {mode}
              </button>
      ...
      <div className="mt-4 flex flex-col gap-1.5 border-t border-white/10 pt-3 text-[12.5px] font-normal">
        ...
        <div className="flex justify-between">
          ...
          <span className="tnum text-white">{orderValueUsdc > 0 ? formatUsd(orderValueUsdc) : "N/A"}</span>
        </div>
        <div className="flex justify-between">
      ...
      <div className="mt-4 flex flex-col gap-2 border-t border-white/10 pt-4">
        <button
          ...
          className="cursor-pointer rounded-[14px] bg-up p-3.5 font-sans text-[15px] font-semibold text-up-ink transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {walletId ? "Deposit" : "Preparing…"}
        </button>
        <button
