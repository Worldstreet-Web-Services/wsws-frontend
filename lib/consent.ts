...
// requires, and whether they want product email, which is optional and off
// until they say so. Both are kept in local storage so they survive the
// round trip an OAuth sign in makes, and so a returning person on the same
// device is not asked again.
//
...
export function readConsent(): Consent {
  const store = storage();
  if (!store) return EMPTY_CONSENT;
  try {
    const raw = store.getItem(STORAGE_KEY);
    if (!raw) return EMPTY_CONSENT;
    const parsed = JSON.parse(raw) as Partial<Consent>;
    // An acceptance of an older version of the terms does not carry over.
    const terms = parsed.terms === true && parsed.termsVersion === TERMS_VERSION;
    return {
      terms,
      marketing: parsed.marketing === true,
      acceptedAt: terms && typeof parsed.acceptedAt === "string" ? parsed.acceptedAt : null,
...
/** Always empty on the server, so the markup matches the first client paint. */
 ...
 *
 * Called when a sign in completes. Nothing to send when the terms were not
 * accepted here (the buttons were disabled, so this is a returning session
 * from a device that agreed earlier) or when this account already has these
 * answers. A failure is logged and left for the next sign in; it never blocks
 * the person from getting to the app.
 */
export async function recordConsent(userId: string): Promise<boolean> {
  const consent = consentSnapshot();
  if (!consent.terms || !consent.acceptedAt || consent.recordedFor === userId) return false;
