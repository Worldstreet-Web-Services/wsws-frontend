...
export function KashUpgradeModal({ open, onClose }: KashUpgradeModalProps) {
  ...
  // Real-USDC mode requires an on-chain payment the app does not ship yet
  // (same leg as purchases), so paid tiers stay visible but disabled with an
  // honest note rather than a request that can only fail.
  const paymentUnsupported = status?.treasury.usdcMode === "ethers";

  const pick = async (tier: number) => {
    if (!wallet || subscribe.isPending || paymentUnsupported) return;
    try {
      await subscribe.mutateAsync({ wallet, tier });
  ...
  return (
    <ModalShell open={open} onClose={subscribe.isPending ? () => {} : onClose}>
                ...
                >
                  ...
                  {isCurrent ? (
                    ...
                    <button
                      ...
                      disabled={subscribe.isPending || paymentUnsupported}
