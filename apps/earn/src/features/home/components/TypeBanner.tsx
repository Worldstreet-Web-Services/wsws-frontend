...
const banners: Record<TypeTypes, TypeBanner> = {
  bounties: {
    ...
    description:
      'Complete bounties and earn cryptocurrency. Browse the latest web3 bounties from top TSION projects — from development and design to content creation. Get paid in USDC, SOL, and other tokens.',
  },
  projects: {
    img: ASSET_URL + '/banner/banner',
    heading: 'Crypto Jobs & Web3 Opportunities',
    description:
      "Discover the best remote crypto jobs and web3 careers in the TSION ecosystem. Whether you're a developer, designer, or content creator, find bounties, freelance projects, and opportunities that pay in crypto.",
  },
};
      ...
      <div className="absolute top-1/2 flex w-full max-w-7xl -translate-y-1/2 flex-col items-start px-2 lg:px-6 xl:px-0">
        ...
        {!user && (
          <AuthWrapper className="w-full sm:w-auto">
            <Button
              className="ph-no-capture text-brand-grey mt-5 h-9 w-full bg-white px-9 py-1 text-sm sm:w-auto md:h-10 md:py-3"
              onClick={() => {
