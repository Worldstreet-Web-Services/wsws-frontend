...
import { type Token, useTokenList } from '@earn/constants/tokenList';
...
import { getRankLabels } from '@earn/utils/rank';
...
import { calculateTotalPrizes } from '@earn/features/listing-builder/utils/rewards';
    ...
    if (isLoading) {
      return (
        <div className="flex justify-center">
          <div className="border-t-brand-grey h-8 w-8 animate-spin rounded-full border-4 border-slate-200" />
        </div>
      );
    }
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      {!showMultiplePayments && (
                        <Button
                          type="button"
                          variant="link"
                          className="text-brand-grey hover:text-brand-grey-dark w-fit p-0 text-sm"
                          onClick={() => {
