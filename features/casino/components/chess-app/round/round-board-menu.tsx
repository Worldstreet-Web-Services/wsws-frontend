...
export function RoundBoardMenu({
  ...
  // The menu unfolds rather than snapping. Positioning lives on the wrapper
  // because Disclosure's own className lands on its inner clip box, and an
  // `absolute` there would take the box out of the grid row that animates it.
  return (
    <div className="absolute inset-x-0 top-[2.5rem] z-30">
      <Disclosure
        open={open}
        id="chess-board-menu"
        className="overflow-hidden rounded-b-[7px] border-t-2 border-[#629924] bg-[#262421] shadow-[0_8px_24px_rgba(0,0,0,0.42)]"
      >
        <div role="menu" aria-label="Board menu">
          <button
            type="button"
            role="menuitem"
            onClick={onFlip}
