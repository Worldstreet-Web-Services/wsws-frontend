...
// Ported from the vengenceui "Twisting Ribbon"; the `cn` dependency was dropped
// and the default palette retinted to a violet-on-black scheme that blends with
// the dark page (both light and dark branches share it, since the app has no
// explicit `dark` class).
...
export function TwistingRibbon({
  ...
  useEffect(() => {
    ...
    const L_COLOR_FOLD_B = lightColors?.foldB ? hexToRgb(lightColors.foldB) : V_FOLD_B;
    const L_COLOR_FOLD_C = lightColors?.foldC ? hexToRgb(lightColors.foldC) : V_FOLD_C;
    const L_SHADOW_COLOR = [0, 0, 0];
    const L_SHADOW_ALPHA = 120 / 255;
    const L_EDGE_COLOR = [255, 255, 255];
    const L_EDGE_ALPHA = 30 / 255;

    const D_COLOR_FACE = darkColors?.face ? hexToRgb(darkColors.face) : V_FACE;
    const D_COLOR_FOLD_A = darkColors?.foldA ? hexToRgb(darkColors.foldA) : V_FOLD_A;
    const D_COLOR_FOLD_B = darkColors?.foldB ? hexToRgb(darkColors.foldB) : V_FOLD_B;
    const D_COLOR_FOLD_C = darkColors?.foldC ? hexToRgb(darkColors.foldC) : V_FOLD_C;
