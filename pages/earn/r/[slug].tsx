...
export default function ReferralLandingPage() {
  ...
  const redirectReason = useMemo(() => {
    ...
    if (data?.inviter?.id && user.id === data.inviter.id) {
      return 'self_referral' as const;
    }
    if (user.isTalentFilled) {
      return 'existing_user' as const;
    }
    return null;
  }, [authenticated, data?.inviter?.id, user]);
  ...
  const headline = useMemo(() => {
    ...
    if (!data) return '';
    if (data.valid)
      return `${data.inviter?.name ?? 'A Superteamer'} has invited you to TSION Earn`;
    if (data.reason === 'INVALID')
