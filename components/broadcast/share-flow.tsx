...
export function ShareFlow({ target, onClose }: { target: BroadcastTarget; onClose: () => void }) {
  ...
  return (
    // Portalled: the sidebar this can open from is translated, and a transformed
    // ancestor would make `fixed` mean the sidebar rather than the viewport.
    <Portal>
        ...
        >
          {profile.isPending ? (
            ...
            <>
              ...
              <p className="mt-1.5 text-[12.5px] leading-[1.55] text-white/55">
                Only creator accounts can start a Market Square stream. Apply for the creator role
                and you can broadcast once it is approved. You can still join a broadcast someone
                else started.
              </p>
              <button
                type="button"
            ...
            <>
              ...
              <p className="mt-3 rounded-[12px] border border-amber-400/30 bg-amber-400/10 px-3 py-2.5 text-[12px] leading-[1.5] text-amber-100">
                Never share a screen while a seed phrase or private key is visible. Ark cannot
                detect these, and anything on the surface you pick goes out live.
              </p>
              <p className="mt-2 text-[11.5px] leading-[1.5] text-white/45">
                Ark does not offer your entire screen: the picker will only let you choose a tab or
                a window.
              </p>
