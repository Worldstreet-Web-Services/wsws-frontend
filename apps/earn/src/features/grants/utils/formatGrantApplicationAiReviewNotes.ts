...
export const formatGrantApplicationAiReviewNotes = (
  ...
  if (review.technicalReview?.isTechnical) {
    appendBullet(
      lines,
      'Technical read',
      [
        review.technicalReview.summary,
        review.technicalReview.technicalCoherence !== 'not_applicable'
          ? `coherence: ${review.technicalReview.technicalCoherence}`
          : undefined,
      ]
        .filter(Boolean)
        .join(' '),
    ...
    appendListBullet(
      ...
      review.technicalReview.missingImplementationDetails,
    );
  }
