...
describe("a drawn game", () => {
  ...
  it("renders spectator chat and sends a new chat line from the play rail", async () => {
    ...
    await waitFor(() =>
      expect(chessApi.postMatchChatMessage).toHaveBeenCalledWith("m1", "spectator", "nice move", null)
    );
  });
  ...
  it("loads and saves a private note plus current-position comments", async () => {
    ...
    await waitFor(() => expect(chessApi.saveMatchNote).toHaveBeenCalledWith("m1", "updated prep", null));
