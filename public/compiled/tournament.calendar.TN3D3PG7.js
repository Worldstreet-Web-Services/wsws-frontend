...
// ../tournament/src/view/calendarView.ts
...
function renderLane(tours, day) {
  return h(
    "lane",
    tours.map((t2) => renderTournament(t2, day))
  );
}
...
function makeLanes(tours) {
  const lanes = [];
  tours.forEach((t2) => {
    const lane = lanes.find((l) => fitLane(l, t2));
    if (lane) lane.push(t2);
    else lanes.push([t2]);
  });
  return lanes;
}
function renderDay(ctrl) {
  return function(day) {
    ...
    const tours = ctrl.data.tournaments.filter((t2) => t2.bounds.start < dayEnd && t2.bounds.end > day);
    return h("day", [
...
function renderTimeline() {
  const hours = [];
  for (let i2 = 0; i2 < 24; i2++) hours.push(i2);
  return h(
    "div.timeline",
    hours.map(
      (hour) => h(
...
function makeGroups(days) {
  ...
  for (let i2 = 0; i2 < days.length; i2 += chunk) groups.push(days.slice(i2, i2 + chunk));
  return groups;
}
function calendarView_default(ctrl) {
...
// ../tournament/src/tournament.calendar.ts
...
function initModule(opts) {
  ...
  opts.data.tournaments.forEach((t2) => {
    if (!t2.bounds)
      t2.bounds = {
        start: new Date(t2.startsAt),
        end: new Date(t2.startsAt + t2.minutes * 60 * 1e3)
      };
  });
  const ctrl = {
    data: opts.data
  ...
  let vnode;
  function redraw() {
    vnode = patch(vnode || element, calendarView_default(ctrl));
  }
  redraw();
}
export {
...
//# sourceMappingURL=tournament.calendar.TN3D3PG7.js.map
