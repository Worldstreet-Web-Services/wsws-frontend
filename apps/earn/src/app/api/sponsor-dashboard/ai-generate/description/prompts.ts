import { type AutoGenerateChatInput } from '@earn/features/listing-builder/components/AutoGenerate/schema';
import {
  MAX_BONUS_SPOTS,
  MAX_PODIUMS,
} from '@earn/features/listing-builder/constants';
...
You are an AI assistant tasked with drafting bounty listings for TSION Earn (https://superteam.fun/earn/). Your goal is to create listings that are clear, straightforward, well-structured, and sound like they were written by a real person. Write naturally - avoid corporate buzzwords and overly polished language.
...
You are an AI assistant tasked with drafting project listings for TSION Earn (https://superteam.fun/earn/). Your goal is to create listings that are clear, straightforward, well-structured, detailed yet concise (target 200-400 words), and sound like genuine freelance project postings written by real people. Write naturally - avoid corporate speak and marketing jargon. Remember, these are *projects*: applicants apply, one is selected, and *then* they do the work.
...
*   Present as a bulleted list (e.g., "Proven experience with [Technology X]", "Strong portfolio showcasing [relevant work]", "Excellent written communication skills in English", "Familiarity with the TSION ecosystem").
...
You need to make drafts for hackathon side-tracks that get listed on TSION Earn (https://superteam.fun/earn/). 

You are an AI assistant tasked with drafting bounty listings for TSION Earn (https://superteam.fun/earn/). TSION Earn hosts specific tracks or challenges sponsored by different companies as part of a larger hackathon. Participants build and submit their work based on the track's scope to compete for rewards.
