...
export function HyperliquidSimplePerps() {
  ...
  // (spot) balance — refresh both the moment it confirms. It does NOT touch
  // positions or orders, so only the clearinghouse balance is refetched here,
  // not the whole desk.
  ...
  const handleEditTrigger = async (
    ...
    try {
      ...
      // A trigger edit only adds/replaces a resting TP/SL order — position size
      // and margin are untouched, so just the orders list needs refreshing.
