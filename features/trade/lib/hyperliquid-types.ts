...
// shapes by hand. See apps/perp/src/signing/README.md for the signing model these
...
/** The platform fee collected ALONGSIDE a withdrawal: a client-signed HyperCore
 *  `sendAsset` to the perps treasury. Null when no treasury/fee is configured.
 *  Hyperliquid's own ~$1 withdraw3 fee is separate and always applies, so a user
 *  pays this + $1 total. */
...
export interface CctpDepositConfig {
  /** true = the user pays the mint fee (Circle's Forwarding Service relays the
   *  mint, ~$0.24 from the deposit); false = the platform self-relays it free.
   *  The client sizes the burn's maxFee from this. */
  userPaysDepositFee: boolean;
  /** CCTP V2 source domain for deposits (6 = Base). */
  sourceDomain: number;
}
