...
export function KashCard({ onBuy, onSend, onConvert, onHistory, onUpgrade }: KashCardProps) {
  ...
  const { data: account, wallet, walletMissing, isError } = useKashAccount();
  ...
  // An unknown balance must never render as zero: a failed read or a wallet
  // still provisioning is not an empty wallet, and a holder shown "0" reads it
  // as their money gone. These stay distinct from a genuine zero balance.
  ...
  return (
    <div className="ws-card relative flex h-full flex-col overflow-hidden p-5 sm:p-[26px]">
      ...
      <div className="relative mt-3" data-testid="kash-balance">
        {loading ? (
          <>
