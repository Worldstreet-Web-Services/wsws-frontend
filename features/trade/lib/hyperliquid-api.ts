...
// Goes through the app's perp BFF proxy (app/api/perp/[...path]/route.ts) — the
// SAME backend service, just the /ark/* sub-paths. One transport, per this
// app's architecture rules.
...
export async function submitWithdrawal(
  ...
  signature: HlSignature,
  // The platform fee: a second client-signed sendAsset to the treasury, or null
  // when no fee is configured. The backend verifies it and relays withdraw-first,
  // fee-second (see apps/perp's WithdrawalService).
...
// ── CCTP deposit (Base -> HyperCore, one hop) ────────────────────────────
// The burn on Base is signed and broadcast client-side; these back the deposit
// UI: `config` says who pays the mint fee (so the client sizes the burn maxFee),
// `record` logs the burn for tracking, `status` polls it by the burn tx hash.
...
export async function getCctpDepositStatus(txHash: string): Promise<CctpDepositStatus | null> {
  return perp.get<CctpDepositStatus | null>(`/ark/deposit/cctp/status/${txHash}`);
}
