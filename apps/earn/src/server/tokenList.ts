...
import { sortTokenSearchResults } from '@earn/utils/tokenSearch';
...
export type AddVerifiedRegistryTokenResult =
  | {
      type: 'success';
      token: Token;
    }
...
export async function addVerifiedRegistryToken(
  registryToken: RegistryToken,
): Promise<AddVerifiedRegistryTokenResult> {
  if (!registryToken.isVerified) {
    return { type: 'unverified-token' };
  }

  const conflictingSymbolToken = await getTokenBySymbol(registryToken.symbol, {
    includeInactive: true,
  });

  if (
    conflictingSymbolToken &&
    conflictingSymbolToken.mintAddress !== registryToken.id
  ) {
  ...
  const token = await prisma.tokenMetadata.upsert({
    where: { mintAddress: registryToken.id },
    update: {
      tokenName: registryToken.name,
      tokenSymbol: registryToken.symbol,
      icon: registryToken.icon || DEFAULT_TOKEN_ICON,
