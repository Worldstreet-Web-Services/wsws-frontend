...
import { statsDataQuery } from '@earn/queries/hackathon';
import { dayjs } from '@earn/utils/dayjs';
import { roundToNearestTenThousand } from '@earn/utils/number';
...
export function SidebarBanner({ className }: SidebarPosterProps) {
  ...
  return (
    <Link href="/earn/hackathon/world-cup" className="mt-5 block">
      <div
        className={`bg-brand-grey/5 border-brand-grey/4 relative flex h-fit w-full flex-col items-center rounded-lg border p-2 ${className}`}
      >
        ...
        <div className="relative z-10 flex h-full w-full flex-col px-4 pt-2 pb-5 text-black">
          ...
          <p className="relative z-10 mt-3 text-sm leading-[130%] text-[#62748E] md:text-base">
            ...
            TSION. $50K across three tracks: markets, trading agents, and fan
            experiences.
          </p>
