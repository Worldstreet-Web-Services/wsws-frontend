...
import { tweetEmbedLink } from '@earn/utils/socialEmbeds';
...
export const ListingSuccessModal = () => {
  ...
  const listingLink = useCallback(
    (medium?: 'twitter' | 'telegram') =>
      `${getURL()}earn/listing/${slug}/${medium ? `?utm_source=tsionearn&utm_medium=${medium}&utm_campaign=sharelisting` : ``}`,
    [type, slug],
  );

  const tweetShareContent = `Check out my newly added @TSIONGroup opportunity!\n\n${listingLink('twitter')}`;
