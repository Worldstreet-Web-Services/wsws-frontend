...
// The provider prices each game's markets separately, and the time for one
// request grows faster than its size: 24 games took 83 s on staging, past the
// proxy's 45 s limit, while four took about 6 s. The board asks in batches of
// four, all at once, so it waits for the slowest batch rather than the sum.
export const MARKETS_BATCH_SIZE = 4;

async function getBoardMarkets(gameIds: string[]): Promise<SportsbookMarket[]> {
  const batches: string[][] = [];
  for (let i = 0; i < gameIds.length; i += MARKETS_BATCH_SIZE) {
    batches.push(gameIds.slice(i, i + MARKETS_BATCH_SIZE));
  }
  return (await Promise.all(batches.map(getEventMarkets))).flat();
}
...
export const getSportsbookBoard = async (params: EventsQuery): Promise<SportsbookBoardPage> => {
  ...
  const markets = await getBoardMarkets(page.events.map(({ id }) => id));
