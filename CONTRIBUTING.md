...
- `docs/ARCHITECTURE.md` is the shape of the codebase and the live status of the
  restructure. Check it before adding a directory or a shared component.
...
Requires Node 22 or later and pnpm. The version is pinned in `packageManager`;
...
`FATAL ERROR: invalid array length`. Node 22 installs cleanly.
...
| `pnpm format:check` | Prettier, checks only. This is what CI runs |
...
Four layers. Every import points downward. Full detail and the target tree are
in `docs/ARCHITECTURE.md`.

```
app/            routes and API route handlers. Composes features, owns no logic.
features/       one folder per product area. Never imports a sibling's internals.
...
The migration to `features/` is in progress. Until a slice is listed as done in
`docs/ARCHITECTURE.md`, work in it stays where it currently lives. Do not
half-move a feature as a side effect of another change.
...
- Put tests beside the code they cover as `*.test.ts`. The older flat
  `__tests__/` directory is being migrated.
...
4. Before opening the PR, run all four gates locally:
   `pnpm format:check`, `pnpm lint`, `pnpm test`, `pnpm build`. CI runs the same
   four and will reject what you did not check.
