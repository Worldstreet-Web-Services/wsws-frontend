...
// The perp surface: funding (Base -> Arbitrum via Dextopus, plus the CCTP
// Base -> HyperCore deposit rail) and Ark's own reads and prepare/submit write
// pairs (see apps/perp's README for the signing model: every write is signed
// client-side, this backend never forwards a private key).
const PERP_ALLOWED =
...
// Ark's asset registry barely changes; live prices turn over in seconds, so a
// short shared cache collapses concurrent users into one upstream call without
// serving stale marks. Everything else in the surface (margin, positions,
// orders, funding/deposit status) must never be stale.
