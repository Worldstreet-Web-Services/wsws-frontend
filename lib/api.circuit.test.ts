...
describe("apiFetch under an outage", () => {
  ...
  // Recovery must need no ceremony: the first success closes the circuit and
  // ordinary traffic resumes.
  it("reopens the moment a request succeeds", async () => {
    const fetchMock = vi.fn(failing);
    vi.stubGlobal("fetch", fetchMock);
    for (let i = 0; i < 5; i += 1) await apiFetch("/api/prices").catch(() => {});

    // A write gets through, and its success is what tells us the server is back.
