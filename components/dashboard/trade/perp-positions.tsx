      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 font-sans text-[14px] font-semibold">
            ...
            >
              {p.isLong ? "Long" : "Short"} {parseFloat(p.leverage)}x
            </span>
          </div>
          <div className="tnum mt-0.5 text-[12px] font-normal text-white/50">
            {formatUsd(parseFloat(p.initialCollateralUsdc))} margin · entry{" "}
            {formatUsd(parseFloat(p.openPrice))}
            {!isUnsetLevel(p.takeProfit) ? ` · TP ${formatUsd(parseFloat(p.takeProfit))}` : ""}
            {!isUnsetLevel(p.stopLoss) ? ` · SL ${formatUsd(parseFloat(p.stopLoss))}` : ""}
          </div>
        </div>
        {pnl != null ? (
          <span
    ...
    <div className="ws-card overflow-hidden">
      ...
      {loading ? (
        ...
        <div className="border-t border-white/6 px-5 py-6 text-center text-[13px] font-normal text-white/45">
          No open positions. Your live trades appear here.
        </div>
      ) : (
        positions.map((p) => {
