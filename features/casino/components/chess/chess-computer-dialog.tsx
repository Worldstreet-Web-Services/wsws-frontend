...
import type { CreateComputerMatchInput } from "@/features/casino/lib/api/types";
        ...
        if (raw) {
          ...
          if (isStoredSetup(parsed)) {
            setSetup({
              level: parsed.level,
              color: parsed.color,
              coachEnabled: parsed.coachEnabled ?? true,
            });
          }
        }
      } catch {
    ...
    >
      ...
      >
        <button
          type="button"
          onClick={() => void onPlay()}
          disabled={create.isPending || stakeOverBalance || stakeInvalid}
